...
// TAPEURCv2 — Koda Payment Token, Arc Testnet
//
// Change from v1: processCardPayment now burns TAPEURC from the user's wallet
// instead of transferring to a business wallet. This reduces totalSupply by the
// settled amount, creating a verifiable surplus in TAPEURCWrapperv2 that can be
...
//
// [REENTRANCY]     processCardPayment is nonReentrant. TAPEURC is a plain ERC20
...
//                  A blacklisted user retaining the ability to exit (burn TAPEURC
//                  and reclaim EURC via the wrapper) is the correct compliance
...
contract TAPEURCv2 is ERC20, ERC20Burnable, ERC20Pausable, AccessControl, ReentrancyGuard {
    ...
    uint256 public globalDailyLimit = 10_000 * 10 ** 6; // 10,000 TAPEURC
    ...
    // v2: `to` removed — TAPEURC is burned on settlement, not transferred.
    ...
    // Reduces totalSupply, creating a withdrawable surplus in TAPEURCWrapperv2.
    // The `paymentId` in this transaction's calldata is the on-chain proof linking
    // this burn to a specific Stripe authorisation.
        ...
        whenNotPaused
    {
        ...
        // Burn settled TAPEURC — reduces totalSupply, grows wrapper surplus
        _burn(from, amount);
